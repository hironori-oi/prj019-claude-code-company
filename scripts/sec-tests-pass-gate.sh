...
usage() { echo "Usage: $0 --suite <harness|workspace|openclaw-runtime> --pass <int> [--promote]"; exit 2; }

SUITE=""; PASS=""; PROMOTE=0
while [[ $# -gt 0 ]]; do
  case "$1" in
...
slack_alert() {
  ...
  if [[ -n "${SLACK_WEBHOOK_URL:-}" ]]; then
    # curl --fail で 4xx/5xx 時に exit code 反映、network 失敗時も gate 自体は continue
    curl -fsS -X POST -H 'Content-type: application/json' \
      --data "{\"text\":\"[Sec gate] ${msg}\"}" "${SLACK_WEBHOOK_URL}" >/dev/null 2>&1 || true
  fi
}
